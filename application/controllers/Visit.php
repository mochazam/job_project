...
class Visit extends CI_Controller {
	...
	public function __construct() {
		parent::__construct();
        $this->load->model(array('M_visit', 'app_model'));
	...
	public function index()
	{
		...
		$data['dept'] = implode(',', $this->M_visit->getDeptById($this->session->userdata('id_dept')));
	...
	function insert() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $this->M_visit->get_insert($data);
            redirect(base_url() . 'visit/show', 'refresh');
	...
	function show() {
		$this->app_model->getLogin();
		$data['visit'] = $this->M_visit->getVisitDataBy($this->session->userdata('id_karyawan'));
		// $data['visit'] = $this->M_visit->getVisitDataBy(80);
		$data['dept'] = implode(',', $this->M_visit->getDeptById($this->session->userdata('id_dept')));
		$this->load->view('visit/list_visit', $data);
	}

	// function show_map($param1 = '', $param2 = '') {
	// 	$data['lat'] = $param1;
	// 	$data['long'] = $param2;
	// 	$this->load->view('visit/map_visit', $data);
	// }
	...
	function history() {
		...
		if ($submit == 'Tampil') {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$data_visitor = $this->M_visit->getAllHistory($name_id, $start, $end);
				$data['history_visit'] = $this->M_visit->getAllHistory($name_id, $start, $end);

				// var_dump($data_visitor->num_rows());
				// die();
				...
				if ($data_visitor->num_rows() > 0) {
					...
					$multi_marker = $this->M_visit->getAllMarker($name_id, $start, $end);
		...
		$data['list_dept'] =  $this->M_visit->getDept();
		$this->load->view('visit/history_visit', $data);
	}

	function getNama($param = '') {
		$nama = $this->M_visit->getNamaKaryawan($param);
		return $nama;
	}

	function getDept($param = '') {
		$dept = $this->M_visit->getDeptName($param);
		return $dept;
	}

	function getJabatan($param = '') {
		$jabatan = $this->M_visit->getJabatanById($param);
		return $jabatan;
	}
	...
	public function get_name() {
		...
		$tmp 	= '';
		$data 	= $this->M_visit->getNameById($dept_id);
		if(!empty($data)) {
	...
	function getHistoryVisit() {
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$this->load->view('visit/history_visit_list', $data);
	...
	function update(){
		...
		$this->M_visit->update($id,$value,$modul);
		echo "{}";
	}
