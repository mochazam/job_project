...
class Addkaryawan extends CI_Controller {
    ...
    public function insert(){
        ...
        $this->form_validation->set_rules('imgkedua', 'imgkedua', 'required');

        $this->load->library('upload');
        
        $idne = $this->M_addkaryawan->ambil_idterakhir() + 1 ; //untuk mengambil nilai id terbaru

        $nmfile =  $idne . ','. date("d-m-Y");//nama file saya beri nama langsung dan diikuti fungsi time
        ...
        if ($_FILES['ft_profil']['name']) {
            if($_FILES['ft_profil']['name'])
            {
                if ($this->upload->do_upload('ft_profil'))
                {
                    ...
                    // upload gambar 2
                    $this->upload->do_upload('imgkedua');
